...
class MovieFinderUITests: XCTestCase {
	...
	func testSearchFromCache()  {
		...
		app.tables["cache_list"].staticTexts["Ip Man"].tap()
		
		XCTAssert(waitForResultLoading(), "Search from cache is not working")
	}
